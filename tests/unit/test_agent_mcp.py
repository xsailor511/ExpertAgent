...
def test_agent_auto_connects_from_config():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        with patch(
            "coding_agent.tools.mcp.config.load_mcp_config", return_value=config
        ), patch(
            "coding_agent.tools.mcp.config.find_mcp_config",
...
def test_agent_no_mcp_config_is_safe():
    ...
    with patch(
        "coding_agent.tools.mcp.config.find_mcp_config", return_value=None
    ):
        agent = Agent.from_settings()
